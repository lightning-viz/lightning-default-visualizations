...
var ROIViz = function(selector, data, images, options) {
    ...
    utils.fetchData(this, ['timeseries', 0], function(err, data) {
        if(!err) {
            line = new LineChart(selector + ' #line-chart', Array.apply(null, new Array(data.length)).map(Number.prototype.valueOf,0), null, {width: $(selector).width(), height: 300});
        } else {
            line = new LineChart(selector + ' #line-chart', [], null, {width: $(selector).width(), height: 300});
        }
    });
